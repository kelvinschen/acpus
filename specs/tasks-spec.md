...
`@acpus/tasks` provides reusable Acpus task tokens for common local automation. The package owns task-domain input/output contracts and typed recoverable task-domain refusals; runtime scheduling and task execution belong to `@acpus/runtime`.
...
- `createWorktree.fn(...)` MAY remain a throwing compatibility adapter over `tryCreateWorktree(...)` for the task runtime boundary.
...
- Tests MUST cover public git subpath exports.
