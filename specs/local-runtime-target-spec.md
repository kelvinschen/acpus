...
- `acpus visualizer` without a Run ID MUST open a picker backed by Run listing; it MUST NOT require a multi-Run dashboard.
- `acpus visualizer <runId>` and `acpus run --visualizer` MUST open the single-Run visualizer view for the selected or submitted Run.
...
- Runtime tests MUST cover that the Run Supervisor exits after five idle minutes with no running Runs or active visualizer/follower clients.
...
- Runtime tests MUST cover Run listing and the visualizer picker returning the most recent 50 Runs sorted by `updatedAt` descending.
