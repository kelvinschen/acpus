...
- The `# OUTPUT SCHEMA` section MUST include the instruction `After completing the task, your final response MUST be exactly one JSON object that conforms to this schema, with no Markdown or prose. Extra keys are accepted but are not available to later workflow expressions.` before the schema serialized as pretty JSON.
...
- When an Agent or Program Step output contains extra fields beyond its declared output schema, the interpreter MUST preserve the full output in persisted Node state and MUST expose only declared schema fields in the workflow expression context and composite parent outputs. A bare `object` output field has no declared child fields, so it MUST project as an empty object in expression context; authors MUST declare nested fields when workflow logic needs them.
...
- The runtime MUST support delivering an external decision payload to a Signal Node that is `awaiting`, addressed through an explicit node key parameter (`--node <nodeKey>` in the CLI). When the Signal Node declares an `output` schema, the payload MUST validate strictly against it, including rejecting undeclared extra fields, and a non-conforming payload MUST be rejected without resolving the Node; when no schema is declared any payload object is accepted.
