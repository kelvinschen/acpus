...
- The full Artifact viewer MUST occupy `100dvw × 100dvh` at every viewport size without a surrounding mask, outer border, shadow, or canvas inset. It MUST retain a fixed metadata-and-action header and an independently scrolling document body.
...
- The full Artifact viewer MUST enter within 160ms and exit within 120ms using opacity only, and remove its custom motion under `prefers-reduced-motion`.
...
- A Runtime store notice MUST stay hidden while the store is ready. When attention is required it MUST use restrained emphasis, one short explanation, and at most one primary `Fix` action with no extra step or internal store details.
- While repair is pending, the `Fix` action MUST remain disabled and retain its position. A failed repair MUST leave a concise error and a retry action in the same notice; an unavailable store MUST not offer an action that cannot succeed.
