...
- Maintenance trigger: update this spec when changing commands, flags, JSON output behavior, lifecycle semantics, saved workflow layout, or command error behavior

## Purpose

The CLI is the developer and Main Agent entry point for planning, running, observing, monitoring, diagnosing, resuming with --force, saving, listing, and showing workflows.
...
- `plan --quiet` MUST set exit code 1 on validation failure.
- The `<spec>` positional argument MUST auto-detect whether the value is a file path or a saved workflow name.
- Auto-detection MUST check: (1) if the value contains a path separator or has a recognized extension (.json, .spec.json), treat as file path; (2) otherwise, treat as a saved workflow name.
...
- `follow` MUST stream events from `events.ndjson` in real-time until the run reaches terminal status.
- `follow --json` MUST output events as NDJSON lines.
...
- `resume --force` MUST bypass the active-worker check to allow restarting a stale worker.
...
- `save --template <name>` MUST generate a spec from a template and save it in one step.
- `save --template` MUST be mutually exclusive with providing a spec argument.
- `list` MUST support `runs` and `workflows` subcommands.
- `show` MUST support `run` and `workflow` subcommands.
- The drafts directory and draft kind MUST NOT be created or referenced.
...
```bash
skills/acpus/scripts/acpus plan workflows/examples/simple-feature.workflow.spec.json
skills/acpus/scripts/acpus plan workflows/examples/simple-feature.workflow.spec.json --json
skills/acpus/scripts/acpus plan workflows/examples/simple-feature.workflow.spec.json --quiet
skills/acpus/scripts/acpus plan simple-feature
skills/acpus/scripts/acpus run workflows/examples/simple-feature.workflow.spec.json
skills/acpus/scripts/acpus run workflows/examples/simple-feature.workflow.spec.json --wait
skills/acpus/scripts/acpus run simple-feature
skills/acpus/scripts/acpus save simple-feature workflows/examples/simple-feature.workflow.spec.json
skills/acpus/scripts/acpus save simple-feature workflows/examples/simple-feature.workflow.spec.json --overwrite
...
skills/acpus/scripts/acpus resume --force <logical-run-id>
skills/acpus/scripts/acpus resume --force --json <logical-run-id>
...
- `plan --json` MUST include `ok`, `validation`, and `plan` fields.
...
- `resume --force --json` MUST include `ok`, `runId`, `runDir`, `worker`, and `message`.
- `resume --json` MUST include `ok`, `runId`, `status`, `worker`, and `message`.

## Data Model

CLI commands operate on workflow specs, saved workflow directories, logical run IDs or run directories, execution plans, run indexes, monitor projections, task detail projections, diagnostics projections, and JSON command output envelopes.
...
Lifecycle commands route through the same schema, compiler, run-index, runtime, and projection modules used by non-CLI entry points. Commands that observe existing runs sync persisted artifacts before rendering output. Commands that mutate workflow execution are limited to explicit run and resume paths. Non-wait run starts a background worker. Wait-style run and resume execute in the foreground and pass the scheduler option that enables fanout draining.
...
- Lifecycle commands -> `src/commands/plan.ts`, `src/commands/run.ts`, `src/commands/run-worker.ts`, `src/commands/follow.ts`, `src/commands/monitor.ts`, `src/commands/resume.ts`, `src/commands/diagnose.ts`
