...
- Runs MUST expose a single known-workspace selector whose default is the launch workspace. The selector MUST scope the Runs list and Run Monitor only; Workflows, server config, health, and sidebar Runtime status MUST retain launch-workspace semantics. The selection MUST remain application-local, MUST NOT alter the URL or browser history, MUST NOT persist across reloads, and MUST NOT offer an all-workspaces aggregate.
