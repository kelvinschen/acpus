...
- Parallel/Fanout concurrency resolution MUST treat missing values and zero as no authored local cap, accept positive integers, and return typed errors for negative, fractional, or non-number values. Quorum count resolution MUST continue to require a positive integer.
...
- Parallel/Fanout `maxConcurrency` and Fanout quorum `count` MUST resolve once when the group materializes. A positive concurrency limit and the concrete quorum count MUST be stored in `group.started`, rebuilt into group projection state after recovery, and used by concurrency and completion policy from that projection. Missing or zero concurrency MUST omit `maxConcurrency` from the event and projection so the daemon and ancestor limits remain effective.
...
- Tests MUST cover missing, zero, positive, negative, fractional, and wrong-type composite concurrency resolution; zero concurrency MUST keep groups progressing without a persisted local cap, while zero quorum count MUST remain invalid.
