...
- Acpus authoring rules MUST reject expression callbacks unless the callback is an inline one-expression arrow with the helper's expected parameter count: one for `fmap`, two for `lift2`, three for `lift3`, and one named dependency parameter for `lift`.
- Acpus authoring rules MUST reject expression callbacks that are block bodies, normal function expressions, generator function expressions, helper references, callbacks with invalid parameter shape, `this`, or references to workflow/module lexical bindings outside callback parameters and nested callback parameters.
- Acpus authoring rules MUST allow ordinary JavaScript syntax inside expression callbacks when it does not introduce external lexical captures, including ordinary methods, nested expression-body arrows, runtime globals such as `Math`, `JSON`, and `Date`, assignments, `new`, and dynamic imports. TypeScript and output-admissibility checks own non-WorkflowData return types.
...
- Tests MUST cover expression callback authoring diagnostics for accepted `fmap`/`lift` one-expression callbacks, globals and ordinary methods, and rejected block bodies, captures, helper references, invalid arity, and function expressions.
