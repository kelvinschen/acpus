...
- Agent discovery MUST declare an explicit role, prompt, and item limit.
- `fanout` MUST schedule one independent runtime item per selected item while respecting global and stage concurrency limits.
...
- `limits.maxAgents` MUST account for worst-case planned agent calls, including fanout item calls and one possible output-repair call per agent invocation.
- `limits.maxFixRounds` MUST bound accepted fix-loop rounds at compile time.
...
Limits include `maxAgents`, `maxConcurrency`, `maxFanoutItems`, `maxFixRounds`, `stageTimeoutMinutes`, and `maxOutputChars`.
