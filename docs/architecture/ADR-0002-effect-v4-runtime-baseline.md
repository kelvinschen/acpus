...
ADR-0001 establishes Effect as the foundation for Acpus effectful
application/runtime code while preserving Acpus-owned durable scheduler,
persistence, fencing, retry, replay, and workflow semantics.
...
other areas that the Acpus migration will touch deeply.

Effect v4 has entered its release-candidate publication line. Because this
branch is intentionally treated as greenfield and the migration is expected to
be completed coherently by coding agents rather than maintained as a long-lived
human-operated dual stack, implementing the new architecture on v3 and then
performing a second structural migration to v4 would add substantial work with
little architectural value.

## Decision

Acpus adopts **Effect v4** as its target Effect generation and executes the
repository-wide migration directly against the **Effect v4 RC** release line.

Effect v3 is not an intermediate implementation target and no v3 compatibility
layer will be maintained.

During the RC period, the coordinating/master agent selects an exact compatible
RC release train and pins it. The exact transient RC patch is an execution
...
- semver ranges must not silently advance the RC beneath active work packages;
...
- changing the pinned RC is a deliberate coordinating change followed by
  typecheck/tests and review before parallel migration work resumes.
...
ADR-0001. An unstable module requires an explicit named work-package decision
that explains:
...
Every Effect migration work package must identify:
...
A work package is rejected if it obtains passing types by emulating obsolete v3
APIs, mixing Effect generations, or hiding an unstable dependency without the
...
`docs/effect-migration/v4-rc-baseline.md` contains the active execution rules for
RC pinning and API source-of-truth behavior. ADR-0001 remains authoritative for
where Effect belongs and where it does not.
