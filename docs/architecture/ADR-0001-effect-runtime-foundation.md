...
- Either / Option / plain ADTs when useful
            |
            v
...
`neverthrow` is removed from the repository as part of this migration. New
`ResultAsync` APIs are forbidden. Pure code may use plain discriminated unions,
`Either`, `Option`, or direct values as appropriate.

Nested failure channels such as `Effect<Result<...>>` or
`Effect<Either<...>>` are not used merely to represent Effect failure. They are
valid only when the nested value is itself intentional domain data.
...
unstructured detached tasks are migration targets, not patterns to reproduce
inside Effect.
...
AgentExecutor, AcpSessionManager, or ProcessService. The application layer
...
- Significant one-time migration across packages currently using neverthrow or
...
The mandatory execution rules are maintained in
`docs/effect-migration/agent-execution-manual.md` and
`docs/effect-migration/review-and-quality-gates.md` while the migration is
active. The accepted architectural boundaries in this ADR remain valid after
the migration documents are retired.
