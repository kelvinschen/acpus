...
- **Agent response repair level:** schema-backed agent response repair is
  ...
  responses; those remain distinct runtime classifications.
...
- **Output failure abstraction:** JSON recovery failure and SchemaIR validation
  ...
  `empty_response` for diagnostics.
...
For schema-backed agent nodes, prompt construction should align with the legacy
contract:
...
The output schema section should state that after completing the task, the final
response must be exactly one JSON object that conforms to the schema, with no
Markdown or prose. It should also state that extra keys are accepted but are not
available to later workflow expressions. The section then includes the
SchemaIR-derived JSON Schema.

Previous output conformance or empty-response error details belong in runtime
diagnostics and artifacts. They should not be injected into the repair prompt in
this goal.

Plain continuation after operator pause/resume or manual control-plane retry
must reuse the same acpx session identity and send only the fixed continuation
prompt. It must not duplicate the full original prompt and must not append the
schema contract. The original task and schema instructions are expected to
remain in the ACP session. Operator pause/resume and manual control-plane retry
are not agent response repair turns and do not consume `retry.max`. A
control-plane rerun that creates a new scheduler-visible attempt gets its own
agent response repair budget.
...
Every acpx turn should ensure the target session exists before sending a prompt.
This aligns with legacy's idempotent `sessions ensure` contract and lets acpx
own saved-session recovery.

When `agentMode` is present, `@acpus/agent-executor` should ensure the acpx
session exists, call acpx `set-mode` for the resolved session before the initial
prompt turn, then send the prompt. Repair and plain-continuation turns should
...
`empty_response` is repairable with the same schema-backed agent response repair
budget when all of these are true:
...
must not be reclassified as `empty_response`. This includes session ensure
failure, set-mode failure, spawn failure, timeout, non-zero provider exit,
output overflow, and detectable JSON-RPC/backend error output.
...
- Expected final spec deltas include:
  ...
  - `specs/runtime-spec.md`: runtime-owned agent-node response repair,
    schema-backed agent output projection, per-turn artifacts, telemetry, and
    scheduler attempt boundaries.
...
- Remove runtime dependencies on `getProviderCommandFromEnv(...)` and provider
  command env mappings from real agent execution paths.
- Ensure scheduler-backed execution and any remaining non-scheduler control
  fallback cannot execute an agent outside the acpx-backed path.
- Update `packages/runtime/src/control/apply-command.ts`,
...
- Add regression coverage proving `agent.use` does not consult provider-command
  env configuration.
...
- Run acpx `sessions ensure` before every prompt turn: initial, repair, and
  plain continuation.
- Build acpx arguments for named agents and `--agent` custom commands.
- Apply model, agent mode, cwd, timeout, permission mode, env, and acpx output
  format.
- If `agentMode` is present, ensure the target session exists and call acpx
  `set-mode <agentMode> -s <session>` before the initial prompt turn.
- Classify acpx `set-mode` rejection or unsupported mode errors as `config`;
  do not send the prompt after a failed mode application.
- Stream and parse acpx output without buffering unbounded data.
- Preserve bounded provider error detail from acpx stdout/stderr, including
  JSON-RPC error messages and non-NDJSON stdout tails, without exposing raw ACP
  protocol lines to runtime as decision input.
- Expose normalized telemetry and final turn result. Do not expose raw ACP
  lines to runtime.
- Return classified execution results instead of throwing for normal acpx
  backend failure outcomes.
- Normalize detectable unsupported model/capability negotiation failures as
...
- Persist and restore dynamic value context for agent leaves so pause/resume and
...
- Apply explicit precedence for permission mode, agent mode, cwd, env, and
  model as described in runtime integration.
- Append schema contract text for schema-backed agent prompts.
- Produce legacy-aligned response repair prompts and plain continuation prompts
  for pause/resume or manual control-plane retry.
- Scrub and set runtime-owned environment variables before invoking acpx.
- Call the agent output conformance helper for balanced JSON recovery,
  conservative JSON repair, schema-backed acceptance, and workflow-visible
  projection.
- Classify exhausted recovery/validation failures as `output_conformance` and
  exhausted repairable empty responses as `empty_response`.
- Treat scheduler attempt timeout as the total budget across all turns and
  repair delays.
- Make fixed repair delays abortable.
- Persist per-turn prompt, response, stderr, telemetry, and attempt aggregate
  metadata.

### E. Core API Cleanup

- Remove `options` from `AgentUseSpec`, `AgentCommandSpec`, and
  `AgentDefinitionIR`.
- Rename agent `policy` to `permissionMode` on authoring specs, run specs, and
  IR.
- Allow `permissionMode` values `approve-reads`, `approve-all`, and `deny-all`.
- Default absent agent `permissionMode` to `approve-all`.
- Add `agentMode?: string` to `AgentUseSpec`, `AgentCommandSpec`, and
  `AgentDefinitionIR`.
- Validate `agentMode` only as a non-empty string when supplied; do not encode a
  mode enum.
- Include the current known mode values as an implementation comment, not a
  validation source.
- Allow `model` on `AgentCommandSpec` and lower it into `AgentDefinitionIR`.
- Add submit/fork agent override types and validation that mirror typed agent
  definition fields and reject `options`.
- Remove or reject `retry.on` and `retry.backoff`.
- Make `retry` invalid on task nodes and on agent nodes without `outputSchema`.
- Keep `retry.max` as extra response repair turn count.

### F. Agent Observability

- Add runtime helpers for agent attempt and turn artifacts using the existing
  artifact registry.
- Persist prompt, response, stderr, telemetry, attempt aggregate metadata, and
  optional raw ACP debug files.
- Enable raw ACP debug files only when `ACPUS_AGENT_RAW_ACP_DEBUG=1`.
- Use structured runtime metadata for attempt/turn manifests and artifact refs;
  avoid storing detailed failure state only in free-form text.
- Attach artifact refs or execution metadata to runtime read projections
  without making scheduler reducers depend on them.
- Expose enough turn failure metadata and artifact/telemetry pointers through
...
- [ ] No public API requires or accepts an external acpx path or provider
  command mapping.
- [ ] No real runtime agent path imports or calls provider-command env mapping
  helpers.
- [ ] Non-scheduler fallback paths cannot execute agents through the old raw
  command protocol.
- [ ] Absent `permissionMode` defaults to `approve-all` and passes explicit
  acpx flags.
- [ ] `permissionMode: "approve-reads"` passes acpx `--approve-reads`;
...
- [ ] `permissionMode` does not define additional Acpus non-interactive
  permission semantics beyond explicit acpx permission-mode flag selection.
- [ ] `permissionMode` does not pass or synthesize acpx
  `--policy`/`--permission-policy` JSON.
- [ ] `agentMode` is accepted on named and custom command agent definitions as
  a non-empty string and is not validated against a hard-coded enum.
- [ ] When `agentMode` is present, executor applies acpx `set-mode` to the
  resolved session before the initial prompt turn.
- [ ] Failed or unsupported `agentMode` application is surfaced as `config` and
...
- [ ] Runtime persists and restores dynamic value context for agent leaves so
...
- [ ] Runtime exposes both rendered explicit session key, when declared, and
  encoded acpx session name through structured agent metadata/read projections.
- [ ] Submit-time and fork-time agent overrides support `use` or `command`,
  `model`, `cwd`, `env`, `permissionMode`, and `agentMode`, reject `options`,
  and inherit across forks according to the frozen run metadata.
- [ ] One scheduler-visible agent attempt may perform multiple acpx turns for
  agent response repair, while scheduler projection still records one leaf
  attempt.
- [ ] Output JSON recovery failure and SchemaIR validation failure are surfaced
  as `output_conformance` in runtime diagnostics.
- [ ] A schema-backed agent turn with empty response text and no backend/tool
...
- [ ] Backend/tool-layer errors are never reclassified as `empty_response` and
  fail directly without agent response repair.
- [ ] Schema-backed agent output accepts extra object keys, stores the raw
  recovered value for diagnostics/artifacts when useful, and exposes only the
  schema-projected `conformedOutput` to node output and expression scope.
- [ ] A schema-backed agent defaults to one initial turn plus two repair turns
  unless explicit `retry.max` overrides it.
- [ ] `retry.max = 0` disables schema-backed agent response repair, including
  `empty_response` repair.
- [ ] Repair turns use the same acpx session and wait a fixed internal 5
  seconds between turns; the delay is abortable and counted in the attempt
  timeout budget.
- [ ] Every acpx prompt turn runs `sessions ensure` first; `agentMode`
  `set-mode` runs only before the initial prompt turn.
- [ ] `retry.on` and `retry.backoff` are not part of public authoring/IR.
- [ ] Task nodes do not support workflow-level automatic retry in this goal.
- [ ] Agent nodes without `outputSchema` cannot declare `retry`.
- [ ] Config failures are not retried.
- [ ] Manual control-plane retry for agent nodes reuses the same acpx session
...
- [ ] Each acpx turn writes independent prompt, response, stderr, and telemetry
  artifacts under the scheduler attempt path and registers them in SQLite.
- [ ] Attempt-level agent telemetry records the list of turns and latest session
  context without making scheduler reducers depend on artifact contents.
- [ ] Runtime never parses raw ACP JSON lines; raw protocol parsing stays inside
  `@acpus/agent-executor`.
- [ ] `options` is removed from agent definition authoring and IR.
- [ ] `policy` is removed from agent definition/run authoring and IR in favor
  of `permissionMode`.
- [ ] Current known agent session mode values are documented in implementation
  comments without becoming validation constraints.
- [ ] `AgentCommandSpec.model` is allowed and applied through acpx/ACP model
  control, not command string mutation.
- [ ] Unsupported command-agent model capability failures are surfaced clearly,
  preferably as `config` when acpx exposes enough detail.
- [ ] acpx stdout/stderr error extraction preserves bounded human-readable
  provider error detail for failures without leaking raw ACP protocol lines into
  runtime decisions.
- [ ] `ACPUS_AGENT_RAW_ACP_DEBUG=1` enables raw ACP debug artifacts; absent or
  other values leave them disabled.
- [ ] Public read APIs expose enough turn failure and artifact metadata for
  CLI/follow to show agent response repair history.
- [ ] Schema prompt text follows the legacy contract: final response exactly one
  JSON object, no Markdown/prose, extra keys accepted but not workflow-visible.
...
- follow/show output distinguishes in-progress repair turns from terminal node
  failure
