...
### 5. Per-Item Subgraph / Map Pipeline
...
1. **Route output alias / selected output binding**
   - 条件分支汇合后的公共 stage 需要稳定读取被选中 route 的输出。
   - 可以避免 prompt 里塞多个 optional variable。

2. **原生 tool/MCP task 节点**
   - 对设计到代码和 research ingest 很关键。
   - 可以先支持 read-only deterministic tool call，再考虑 mutation。

3. **自定义 program/command task 节点**
   - 对验证、构建、截图、确定性 transform 都有用。
   - 需要强约束 sandbox、timeout 和 output contract。

4. **Fanout 后续增强**
   - Lane Group 基础能力已落地；这里保留给 lane group dependencies、lane-level variables 或 partial policy 等增强。

5. **条件汇合 / 普通并行分支/汇合**
   - 后续解锁 top-level `A -> B1/B2 -> C` 和 selected route convergence。

6. **per-item subgraph / 动态 worklist**
