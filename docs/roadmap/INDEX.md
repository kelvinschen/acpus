...
- **Forked Run 拓扑序继承（缺口，F10）**：`planFork` 当前按 checkpoint 写入顺序（终态完成时间）做线性截断。在 parallel/fanout 兄弟乱序完成的场景下，这会导致"实际可继承的 sibling 被截断"——是 *少继承*（语义安全但偏保守），不是错。彻底修复需要按拓扑序 walk 新 IR、对每个 Node 单独决定是否继承。代价是 fork planner 要复刻 interpreter 的控制流推导。先观察实际碰撞频率，必要时再做。
