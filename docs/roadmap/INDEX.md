...
| R4 | 真实 agent 兼容矩阵与风险缓解 | M6 | 待开始（下一个，贯穿） | [R4-real-agent-compat-matrix.md](R4-real-agent-compat-matrix.md) |
...
- **R4 贯穿**：acpx 版本 pin、真实 adapter 兼容矩阵、Open Risks（#1 adapter 差异、#2 acpx alpha、#3 Temporal 内核易用性、#4 artifact 压缩）作为持续项，后续从 R4/backlog 继续收敛。
