...
- **Mock script 依赖边界优化（候选）**：当前 `type: mock` + `mock_script` 是已落地行为，`@acpus/runtime` 在 in-memory `MockAgentExecutor` 中直接复用 `@acpus/mock-agent` 的脚本解析与 prompt 规则匹配能力，因此 `@acpus/mock-agent` 需要作为 runtime 的生产依赖而非 dev 依赖。后续若希望减小 runtime 发布依赖或避免 runtime→mock-agent 包耦合，可考虑把无 I/O 的 mock script DSL、parser、response selection 抽到 `@acpus/core` 或新的 shared 包，`@acpus/mock-agent` 仅保留 ACP stdio server/trace/bin 进程边界；也可统一 CLI mock-agent 与 in-memory executor 的相对路径解析、缓存失效策略和错误分类。该优化不改变当前 `mock_script` 语义，只是发布/包边界整理。
