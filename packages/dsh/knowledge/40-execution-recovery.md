...
Call `acpus_run` with the complete workflow string and `agents` mapping every unbound slot to a string: exact Preset matches take priority over named Agents. For the usual one-shot workflow, embed facts known before admission and omit run input. If a workflow intentionally declares `inputSchema`, pass only matching business input. An `invalid` result creates no task: repair all related diagnostics coherently and resubmit changed source or bindings. When `phase` is `agents`, bind every missing slot using a catalog Preset id, a user-selected Agent name, or direct `use`/`command` fields. An `admitted` result supplies the exact `{ name, occurrence }` selector; preserve it for later operations.
