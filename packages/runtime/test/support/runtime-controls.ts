...
import { openExistingWritableRuntimeStoreAdapter, type RunDetails } from "../../src/store/store.js";
...
export async function forkRun(workspace: string, runId: string, options: ForkOptions = {}): Promise<{ run: RunDetails }> {
  ...
  if (!store) throw new Error("Expected runtime store.");
  try {
    const forkResult = await Effect.runPromise(Effect.result(store.forkRun(runId, options)));
    if (Result.isFailure(forkResult)) {
      throw Object.assign(new Error(forkResult.failure.message), { failure: forkResult.failure });
    }
    const fork = forkResult.success;
