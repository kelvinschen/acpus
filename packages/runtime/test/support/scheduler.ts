...
export async function applySchedulerControlIntent(
...
): Promise<{ runId: string; snapshot: SchedulerSnapshot; effect: SchedulerControlEffect | undefined; advanced?: AdvanceRunSummary }> {
  ...
  try {
    ({ snapshot, effect } = applySchedulerControlIntentWithOwner(store, intent, claim.ownerEpoch));
  } finally {
    store.scheduler.releaseRun(claim);
  }

  if (intent.type === "pause" || options.advance === false) return { runId: intent.runId, snapshot, effect };
