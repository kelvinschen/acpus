...
import { admitWorkflowRun, admitWorkflowRunOnly, advanceWorkflowRun, createWorkflowVisualizationOverlay, getRun, getRuntimeHealth, getRunVisualizationOverlay, listRuns, mutateRun, mutateRunControlOnly, normalizeForkInput, normalizeSignalPayload, normalizeWorkflowInput, queueSupervisorShutdown, releaseWorkflowRunOwner, replayRun, signalRun, signalRunControlOnly, startSupervisorLoop, tryAdvanceRun, tryAdvanceRuntimeRun, tryAdmitWorkflowRun, tryMutateRun, trySignalRun, validateAgentOverrides } from "@acpus/runtime";
...
test("@acpus/runtime public types describe use-case level runtime APIs", () => {
  ...
  expectTypeOf(admitWorkflowRunOnly).toEqualTypeOf<(cwd: string, prepared: PreparedRunWorkflow, input: JsonValue, agentOverrides?: AgentOverrideMap) => Promise<RunDetails>>();
  ...
  expectTypeOf(signalRunControlOnly).toEqualTypeOf<(cwd: string, runId: string, nodeId: string, payload: JsonValue) => Promise<RuntimeMutationResult | undefined>>();
  ...
  expectTypeOf(mutateRunControlOnly).toEqualTypeOf<(cwd: string, runId: string, action: RuntimeMutationAction, input?: RuntimeMutationInput) => Promise<RuntimeMutationResult | undefined>>();
