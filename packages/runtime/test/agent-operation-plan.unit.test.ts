...
describe("Agent Attempt admission planning", () => {
  ...
  it("allows Safe Retry only for the exact not-dispatched prompt", () => {
    ...
    })._unsafeUnwrap()).toMatchObject({ operation: "safe_retry" });
    expect(planAgentAttemptAdmission({
      source: "safe_retry",
      target: "agent~1",
      session,
    ...
    })._unsafeUnwrapErr().type).toBe("safe_retry_input_mismatch");
  });
});
