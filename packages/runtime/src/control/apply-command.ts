...
import type { ForkCommandPayload, PendingRunControlCommand, RuntimeStore } from "../store/store.js";
...
function forkCommandPayload(command: Extract<PendingRunControlCommand, { type: "fork" }>): ForkCommandPayload {
  return command.status === "pending" || command.status === "running" ? command.payload : {};
}
