...
export function layoutWorkflow(model: RenderModel): RenderLayout {
  ...
  const measureItem = (id: string): Size => {
    ...
    if (item.children.length === 0) {
      if (item.type === "container") size = { width: containerMinWidth, height: emptyContainerHeight, flowY: emptyContainerHeight / 2 };
      else if (isCompositeKind(item.kind)) size = { width: compositeMinWidthFor(item), height: compositeMinHeight, flowY: compositeMinHeight / 2 };
      else size = { width: leafWidth, height: leafHeight, flowY: leafHeight / 2 };
    } else {
...
export function projectEdgePath(
  ...
  const start = projectPoint({ x: source.x + source.width, y: boxFlowY(source) }, viewport);
  const end = projectPoint({ x: target.x, y: boxFlowY(target) }, viewport);
  const d = edge.kind === "loop" || end.x < start.x
