...
- **NEVER capture workflow values inside inline Task `exec`. NEVER bypass `run.input`.** Pass dependencies through `run.input` and read them from Task context; use `task.define` for imports or shared code.
