...
use acpus_core::{
    AcpusIr, AgentOverrideWarning, AgentOverrides, CompileOptions, apply_agent_overrides,
    compile_workflow, workflow_source_resolver,
};
use axum::{
    Json, Router,
    body::Bytes,
...
async fn list_nodes(
    ...
    supervisor
        ...
        .read_run_meta(&run_id)
        .map_err(|_| ApiError::not_found("Run not found"))?;
    Ok(Json(json!(supervisor.store.read_nodes(&run_id)?)))
}
...
fn validate_control_transition(current: RunStatus, target: RunStatus) -> Result<(), String> {
    crate::ensure_status(current, target).map_err(|error| error.to_string())
}
...
"#;
    ...
    async fn node_retry_route_tracks_background_retry_as_active_work() {
        ...
        let mut node = crate::create_initial_node_state(
            "workflow/retry".into(),
            "retry".into(),
    ...
    fn graceful_shutdown_pauses_live_runs_and_running_nodes() {
        ...
        let awaiting = store
            .create_run_with_options(&ir, json!({}), RunCreateOptions::default())
            .unwrap();
        let mut signal = crate::create_initial_node_state(
            "workflow/gate".into(),
            "gate".into(),
            acpus_core::IrNodeKind::RunSignal,
            None,
        );
    ...
    async fn signal_route_delivers_payload_and_reports_request_errors() {
        ...
        assert_eq!(state.state, crate::NodeState::Completed);
        assert_eq!(
            state.output,
