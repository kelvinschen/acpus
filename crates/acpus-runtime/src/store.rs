use crate::{
    ...
    RunStatus, rewrite_artifact_run_id, validate_run_id,
};
use acpus_core::{
    AcpusIr, AgentOverrideWarning, AgentOverrides, HookConfigSnapshot, IrNodeKind, SchemaDslError,
    digest_json, validate_json_schema_value,
};
...
impl RunStore {
    ...
    pub fn write_node(&self, run_id: &str, node: &NodeExecutionState) -> anyhow::Result<()> {
        let path = self
            ...
            .join("nodes")
            .join(format!("{}.json", crate::storage_key(&node.node_key)));
    ...
    pub fn read_node(&self, run_id: &str, node_key: &str) -> anyhow::Result<NodeExecutionState> {
        let path = self
            .checked_run_dir(run_id)?
            .join("nodes")
            .join(format!("{}.json", crate::storage_key(node_key)));
    ...
    pub fn inherit_node_from_run(
        ...
        let source_artifacts = source_run_dir
            .join("artifacts")
            .join(crate::storage_key(node_key));
        if source_artifacts.is_dir() {
            let target_artifacts = fork_run_dir
                .join("artifacts")
                .join(crate::storage_key(node_key));
...
fn node_index_entry(node: &NodeExecutionState) -> NodeIndexEntry {
    let storage_key = crate::storage_key(&node.node_key);
    NodeIndexEntry {
        node_key: node.node_key.clone(),
...
mod tests {
    ...
    fn write_node_indexes_bounded_storage_key_for_audit() {
        ...
        let storage_key = crate::storage_key(&node_key);
