use crate::agent_overrides::refresh_agent_metadata;
use crate::expression_scope::{ScopedValidationInput, validate_scoped_expressions};
use crate::{
    AcpusIr, AgentSpec, CompileOptions, CompileResult, CompileSchemaDslOptions, Diagnostic,
    IrBranch, IrExpression, IrNode, IrNodeKind, IrSource, NodeKeyTemplate, OutputMerge,
    compile_schema_dsl, create_schedule, parse_duration_ms, source_digest, validate_schema_value,
};
use cel::Program;
use regex::Regex;
...
fn expand_includes(
    ...
    include_resolver: Option<&crate::IncludeResolver>,
...
fn has_blocking_diagnostics(diagnostics: &[Diagnostic], strict: bool) -> bool {
    diagnostics.iter().any(|diagnostic| {
        diagnostic.severity == crate::DiagnosticSeverity::Error
            || (strict && diagnostic.severity == crate::DiagnosticSeverity::Warning)
    })
}
    ...
    fn compile_ts_fixture_path(path: &Path) -> CompileResult {
        ...
        compile_workflow(
            ...
            CompileOptions {
                ...
                include_resolver: Some(crate::create_include_resolver(Some(&source_path))),
                ..Default::default()
            },
        )
    }
