...
mod tests {
    ...
    fn renders_scalar_template() {
        ...
        assert_eq!(
            render_template("hi ${{ input.name }}", &ctx).unwrap(),
            "hi acpus"
        );
    }
    ...
    fn unknown_reference_returns_error() {
        ...
        assert!(matches!(
            render_template("${{ missing.value }}", &ctx),
            Err(EvalError::Execute(_))
        ));
    }

    #[test]
    fn cel_boolean_condition() {
        let ctx = EvalContext {
        ...
        assert_eq!(
            eval_cel("input.enabled == true", &ctx).unwrap(),
            json!(true)
        );
    }
    ...
    fn cel_path_access() {
        ...
        assert_eq!(eval_cel("input.nested.value", &ctx).unwrap(), json!(42));
    }

    #[test]
    fn invalid_cel_returns_typed_error() {
        let ctx = EvalContext {
            workflow: EvalContext::workflow("t"),
            run_id: "r".into(),
            now: "2026-01-01T00:00:00Z".into(),
        ...
        assert!(matches!(eval_cel("input.", &ctx), Err(EvalError::Parse(_))));
    }
